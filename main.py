...
with st.container():
    ...
    if not st.session_state.alive:
        ...
        # --- PC방 루트 (PUBG / 오디오) ---
            ...
            if st.button("사운드 락 프로그램으로 오디오 밸런스를 잡는다", key="pc_3"): process_action(next_stage='pc_audio_fix')

        elif st.session_state.stage == 'pc_pubg':
            st.subheader("배그 접속 완료. 유선 헤드셋을 착용했습니다.")
            ...
            if st.button("조용히 존버 메타로 간다", key="pc_p2"): process_action(is_ending=True, win_msg="54킬 1등으로 평화주의자 치킨을 먹고 프로게이머로 데뷔했습니다!", win_title="엔딩: 배그의 신")
            if st.button("차량 소리가 들리자마자 LMG를 들고 돌진한다", key="pc_p3"): process_action(fatal=True, fatal_reason="버그로 인해 하늘에서 날아온 UAZ 차량에 깔려 현실에서도 심정지가 왔습니다.", fatal_title="물리엔진의 희생양")
            ...
            if st.button("리얼텍 오디오 콘솔을 강제 삭제 후 재설치한다", key="pc_af2"): process_action(fatal=True, fatal_reason="드라이버가 꼬이면서 윈도우 11 블루스크린이 뜨고, 분노를 참지 못해 키보드를 씹어먹다 기도가 막혀 죽었습니다.", fatal_title="블루스크린 증후군")

        # --- 친구 경모 루트 ---
        elif st.session_state.stage == 'friend_start':
            st.subheader("항상 어딘가 덜렁대고 띨띨한 친구 'ㄱㄱㅁ'를 만났습니다.")
            if st.button("ㄱㅁ에게 마라탕을 사준다", key="fr_1"): process_action(next_stage='friend_mara')
            if st.button("ㄱㅁ와 오토바이를 같이 탄다", key="fr_2"): process_action(fatal=True, fatal_reason="ㄱㅁ가 브레이크 대신 엑셀을 밟아 그대로 강물로 다이빙했습니다.", fatal_title="친구따라 강남 간다")
            if st.button("ㄱㅁ를 무시하고 집에 간다", key="fr_3"): process_action(fatal=True, fatal_reason="삐진 ㄱㅁ가 뒤에서 던진 슬리퍼에 뒤통수를 맞고 뇌진탕으로 사망했습니다.", fatal_title="우정 파괴")

        elif st.session_state.stage == 'friend_mara':
            st.subheader("ㄱㅁ가 펄펄 끓는 마라탕 그릇을 들고 오다가 발이 꼬였습니다!")
            ...
            if st.button("ㄱㅁ를 방패막이로 쓴다", key="frm_2"): process_action(is_ending=True, win_msg="ㄱㅁ는 희생되었지만 당신은 화상을 피했습니다. 우정보다 생존입니다.", win_title="엔딩: 냉혹한 생존자")

        # --- 지하 클럽 루트 (음악) ---
        elif st.session_state.stage == 'club_start':
            st.subheader("어두컴컴하고 습한 언더그라운드 클럽에 들어왔습니다.")
            if st.button("DJ에게 짱깨노래를 틀어달라고 한다", key="cl_1"): process_action(fatal=True, fatal_reason="중국을 싫어하는 몽골 형님들에게 둘러싸여 집단 구타를 당했습니다.", fatal_title="선곡 실패")
            if st.button("어둡고 묵직한 비트에 맞춰 춤춘다", key="cl_2"): process_action(next_stage='club_dance')
            if st.button("구석에서 혼자 중국 팝송들을 흥얼거린다", key="cl_3"): process_action(next_stage='club_dance')

        elif st.session_state.stage == 'club_dance':
            st.subheader("당신의 다크한 취향에 클러버들이 호응하기 시작합니다.")
            if st.button("여자들이 출 것 같은 클럽 스타일의 춤을 춘다", key="cld_1"): process_action(is_ending=True, win_msg="당신은 여자가 되는 선택을 하기로 마음먹고 태국에 가서 성전환 수술을 받았습니다...", win_title="엔딩: 게이")
            if st.button("흥분해서 천장의 조명을 매달려 탄다", key="cld_2"): process_action(fatal=True, fatal_reason="부실한 천장이 무너지면서 500kg짜리 미러볼에 깔려서 뒤졌습니다.", fatal_title="미러볼 압사")
        ...
        # --- 서브컬처 행사 (캐릭터 디자인) ---
            ...
            if st.button("생기 없는 눈을 가진 어른스러운 캐릭터 피규어를 찾는다", key="an_2"): process_action(next_stage='anime_ai')
        ...
        # --- 수학 학원 루트 (유리수, 무리수) ---
            ...
            if st.button("답안지에 오직 '정답' 이라고만 적는다", key="ste_1"): process_action(fatal=True, fatal_reason="선생님이 어이가 없어서 던진 삼각자에 경 베여 출혈사했습니다.", fatal_title="기하학적 살인")
